...
class Ssp {
        ...
        if (isset($request['search']) && $request['search']['value'] != '') {
            ...
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true') {
                    $binding = self::bind($bindings, '%' . self::sanitize($str) . '%', PDO::PARAM_STR);
    ...
    static function custom($request, $table, $primaryKey, $columns, $whereResult = null, $whereAll = null) {
        ...
        $where = self::filter_custom($request, $columns, $bindings);
        $whereResult = self::_flatten($whereResult);
        $whereAll = self::_flatten($whereAll);
        if ($whereResult) {
            $where = $where ?
        ...
        return array(
            ...
            "recordsTotal" => intval($recordsTotal),
            "recordsFiltered" => intval($recordsFiltered),
            "data" => self::data_output_custom($columns, $data),
        );
    }

    /**
     * Searching / Filtering | CUSTOM
     *
     * Construct the WHERE clause for server-side processing SQL query.
     *
     * NOTE this does not match the built-in DataTables filtering which does it
     ...
     *  @return string SQL where clause
     */
    static function filter_custom($request, $columns, &$bindings) {
        $globalSearch = array();
        $columnSearch = array();
        $dtColumns = self::pluck($columns, 'dt');
        if (isset($request['search']) && $request['search']['value'] != '') {
    ...
    /**
     * Create the data output array for the DataTables rows | CUSTOM
     *
     *  @param  array $columns Column information array
     *  @param  array $data    Data from the SQL get
     *  @return array          Formatted data in a row based format
     */
    static function data_output_custom($columns, $data) {
