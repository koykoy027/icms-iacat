...
Class Reports_model extends CI_Model {
    ...
    public function getAgencyBranchesByAgencyId($aParam) {
        $sql = "
            SELECT 
                `iab`.`agency_branch_id` as id, 
                CONCAT(`ia`.`agency_abbr`, ' - ', (`iab`.`agency_branch_name`) ) as `name`
            FROM `icms_agency` `ia`, 
...
//        exit($sql);
    ...
    public function getAgencyBranchesServicesByBranchId($aParam) {
        $sql = "
            
         SELECT 
                `iaso`.`services_id` as `id`, 
            ...
            WHERE 
                ...
                `iaso`.`agency_branch_id` = " . $aParam['agency_branch_id'] . "
                ";
//        exit($sql);
        $aResponse = $this->yel->GetAll($sql);
        return $aResponse;
