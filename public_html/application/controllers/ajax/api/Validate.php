...
//            if ($resp['resp_dob']) {
        ...
        $stored = [];
        $stored['type'] = "getAllStoredVictims";
        $stored['limit'] = 100;
        $stored['page'] = 1;
        if ($aResponse['flag'] == 0) {
            $stored['count'] = 0;
            $stored['total_fields'] = 0;
        } else {
            $stored['count'] = $aResponse['count'];
            $stored['total_fields'] = $aResponse['total_fields'];
        }
        $storedVictim = $this->getAllStoredVictims($stored);

        $result = $storedVictim['total_rel_by_info_id']['list'];
        foreach ($result as $key => $val) {
            $result[$key]['full_name'] = str_replace(',', '', $result[$key]['full_name']);
        }
