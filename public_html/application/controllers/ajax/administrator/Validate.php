...
class Validate extends CI_Controller {
    ...
    public function __construct() {
        ...
        // load models
        $this->load->model('administrator/Validate_model');
    }
     ...
     * Old Validate
     */
    public function _validate($aParam) {
        $aResponse = [];
...
//            $date = DateTime::createFromFormat("U", $aParam['victim_info_dob']);
//            $aParam['victim_info_dob'] = date_format($date, 'Y-m-d');
...
//            }
//            if (count($aResponse['rel_dob']) > 0) {
//                $i = 0;
//                foreach ($aResponse['resp_name'] as $key => $val) {
//                    $c = 0;
//                    foreach ($aResponse['rel_dob'] as $k => $v) {
//                        
//                        if ($val['victim_info_id'] == $v['victim_info_id']) {
//                            $aResponse['resp_dob'][$i] = $val;
//                            $aResponse['resp_dob'][$i]['total_rel'] = $val['rel_full_name'] + $v['rel_dob'];
//                        }
//                        $c++;
//                    }
//                    $i++;
...
//            }
        ...
        if (!empty($aParam['offender_name']) !== false) {
            ...
            foreach ($resp['resp_offenders'] as $key => $val) {
                $resp['resp_offenders'][$i] = $this->Validate_model->getVictimByCaseId($val['case_id']);
        ...
        if ($arr) {
            ...
            foreach ($arr as $key => $val) {
                ...
                $values = $values . "('" . $val['victim_id'] . "', '" . $aResponse['count'] . "', '" . $val['victim_info_id'] . "', '" . $val['rel_desc'] . "', '" . $val['rel'] . "'),";
            }
...
//        return $values;
    ...
    /*
     * Old Get all Validation REsult
     */
    public function _getAllStoredVictims($aParam) {
        $aResponse = [];
        ...
        $aRes['victims'] = $this->Validate_model->getAllVictimWithInfoId($aParam['count_id']);


        // fetch victim without info id
        $aRes['victim_wo_info_ids'] = $this->Validate_model->getAllVictimWOInfoId($aParam['count_id']);

        $i = 0;
        foreach ($aRes['victim_wo_info_ids'] as $key => $val) {
            // get vimtim info id by victim id
            $aRes['victim_wo_info_ids'][$i] = $val;
            $aRes['victim_wo_info_ids'][$i]['icms_validation_victim_info_id'] = $this->Validate_model->getVictimInfoId($val['icms_validation_victim_id']);

            $i++;
        }

        // merge arrays
        $arr = array_merge($aRes['victims'], $aRes['victim_wo_info_ids']);


        $uniqueArray = array_unique(array_column($arr, 'icms_validation_victim_info_id'));
        $output = array_intersect_key($arr, $uniqueArray);
        $resp['count'] = count($output);
        // create pagination
        $resp['resp'] = array_slice($output, $aParam['start'], $aParam['limit']);

        // get each victim details

        if ($resp) {
            $c = 0;
            foreach ($resp['resp'] as $k => $v) {
                // get total by info id
                $aResponse['total_rel_by_info_id'][$c] = $this->Validate_model->getTotalRelByVictimInfoId($v['icms_validation_victim_info_id'], $aParam['count_id']);
                $count = count($aResponse['total_rel_by_info_id'][$c]);

                if ($count == '1') {
                    $aResponse['total_rel_by_info_id'][$c] = $this->Validate_model->getTotalRelByVictimId($v['icms_validation_victim_id'], $aParam['count_id']);
                    $aResponse['total_rel_by_info_id'][$c]['total_2'] = 0;
                } else {
                    $total_2 = $this->Validate_model->getTotalRelByInfoToVictimId($v['icms_validation_victim_id'], $aParam['count_id']);

                    if (empty($total_2) !== false) {
                        $aResponse['total_rel_by_info_id'][$c]['total_2'] = 0;
                    } else {
                        $aResponse['total_rel_by_info_id'][$c]['total_2'] = $total_2;
                    } 
                }

                $final = $aResponse['total_rel_by_info_id'][$c]['total'] + $aResponse['total_rel_by_info_id'][$c]['total_2'];
                $percent = round((float) $final / $aParam['total_fields'] * 100);
                $aResponse['total_rel_by_info_id'][$c]['final_rel'] = substr($percent, 0, -1) . '%';
            ...
            // encrypt victim_id
            foreach ($aResponse['total_rel_by_info_id'] as $k => $v) {
                $aResponse['total_rel_by_info_id'][$k]['icms_validation_victim_id'] = $this->yel->encrypt_param($v['icms_validation_victim_id']);
            }

            $aResponse['count'] = $resp['count'];
        }
...
//        $aResponse['total_rel_by_info_id']['list'] = $this->yel->encrypt_id_in_array($aResponse['total_rel_by_info_id']['list'], 'icms_validation_victim_id');
        $aResponse['flag'] = self::SUCCESS_RESPONSE;
     ...
     * OLd
     */
    public function _getVictimCaseList($aParam) {
        $aResponse = [];
        ...
        $aParam['rel_desc'] = $this->Validate_model->getRelValDescByVictimInfoId($aParam);
        $aParam['rel_desc_2'] = $this->Validate_model->getRelValDescByVictimId($aParam);

        if ($aResponse['victim_info']['victim_id']) {
            $aResponse['victim_info']['victim_id'] = $this->yel->encrypt_param($aResponse['victim_info']['victim_id']);
            $aResponse['desc_list'] = array_merge($aParam['rel_desc'], $aParam['rel_desc_2']);
            $x = 0;
            foreach ($aResponse['cases'] as $key => $val) {
                $aResponse['test'][$x] = $val;
                foreach ($aResponse['desc_list'] as $k => $v) {

                    if ($v['icms_validation_relevance_desc'] == 'rel_employer') {
                        $emp_ids = $this->Validate_model->getEmployerNameRel($aParam['employer_name']);
                        $i = 0;
                        foreach ($emp_ids as $a => $b) {
                            if ($b['employer_id'] == $val['employer_id']) {
