...
class Reports extends CI_Controller {
    ...
    public function __construct() {
        ...
        // load models
        $this->load->model('agency/Reports_model');
    ...
    public function generateVictimReport($aParam) {
        ...
        // for admin only 
        if (isset($aParam['agency_id']) == true && $aParam['agency_id'] != '0') {
            $aParamCon['where'] .= ' 
                AND `v`.`added_user_agency_id` = "' . $aParam['agency_id'] . '"
               ';
            if ($aParam['agency_branch_id'] != '0') {
                $aParamCon['where'] .= " AND `v`.`added_user_agency_branch_id` = '" . $aParam['agency_branch_id'] . "' ";
            }
        } else {
            $aParamCon['where'] .= " AND `v`.`added_user_agency_branch_id` = '" . $_SESSION['userData']['agency_branch_id'] . "' ";
        }
    ...
    public function getBaseReportServicesCasePrimaryCondition($id) {
        ...
        // for agency branch only 
        if ($_SESSION['userData']['agency_branch_is_main'] == '1') {
            $aQuery['select'] = "
                    (`agency_branch_name`)
                ";
            $new_param = [];
            $new_param['where'] = " AND `iab`.`agency_id` = '" . $_SESSION['userData']['agency_id'] . "' ";
            $aQuery['base'] = $this->Reports_model->getAgencyBranchesByAgencyId($new_param);
        } else {
            $aQuery['select'] = "
                    (`service_name_full`)
                ";
            $new_param = [];
            $new_param['agency_branch_id'] = $_SESSION['userData']['agency_branch_id'];
            $aQuery['base'] = $this->Reports_model->getAgencyBranchesServicesByBranchId($new_param);
        }
    ...
    public function getBaseReportServicesCasePrimaryConditionAgency($id) {
        $aQuery = [];
    ...
    public function generateCaseVictimReport($aParam) {
        ...
        if (isset($aParam['agency_id']) == true && $aParam['agency_id'] != '0') {
            ...
            $aParamCon['where'] .= " AND `added_user_agency_branch_id` = '" . $_SESSION['userData']['agency_branch_id'] . "' ";
        }
...
//            }
    ...
    public function generateCaseServicesReport($aParam) {
        ...
        // for agency branch only 
        if ($_SESSION['userData']['agency_branch_is_main'] == '1') {
            $aParamCon['where'] .= " AND `agency_id` = '" . $_SESSION['userData']['agency_id'] . "' ";
        } else {
            $aParamCon['where'] .= " AND `agency_branch_id` = '" . $_SESSION['userData']['agency_branch_id'] . "' ";
        }

        $aResult['graph'] = $this->Reports_model->generateCaseServicesReport($aParamCon);

        if (!isset($aParam['br_primary']) !== true) {
            $aResult['br_primary_agency'] = $this->getBaseReportServicesCasePrimaryConditionAgency($aParam['br_primary']);
            $aResult['date_agency'] = $this->getReportTypeCaseServicesCondition($aParam['report_type']);
            $aParamCon['groupBy'] = $aResult['br_primary_agency']['groupBy'];
            $aParamCon['date'] = $aResult['date_agency']['date'];
            $aParamCon['select'] = $aResult['br_primary_agency']['select'];
            $aParamCon['where'] .= $aResult['br_primary_agency']['where'];
            $aParamCon['limit'] = $aResult['br_primary_agency']['limit'];

            $aResult['base_agency'] = $aResult['br_primary']['base'];
        }
...
//                    echo $sV['date'] . '=' . $new_cur_date . ' | ' . $sV['variable'] . '=' . $ddK;
