...
function getArchivedLegalList(x) {

    var limit = 1;
    var keyword = $('#txt_search-lls').val();

    $.post(sAjaxServiceDetails, {
        type: 'getArchivedLegalList',
        limit: limit,
        page: x.page,
        keyword: keyword
    }, function (rs) {
        ...
        if (rs.data.flag == 1) {
            ...
            var keyword = $('#txt_search-lls').val();
            var sFooter = '';
            var sMessage = 'SORRY, WE COULDN\'T FIND ANY LEGAL SERVICE RELATED TO <span class = "font-italic" >"<span>' + keyword + '</span>"</span>.';
            if (!keyword) {
...
$(document).ready(function () {
    ...
    $('.rs-pagination').delegate('.page-link', 'click', function () {
        ...
        getArchivedLegalList({page: page});
    });
