...
function saveAgencyBranchAddress() {
    ...
    $.post(sAjaxAgencies, {
        ...
        icmsMessage({
            ...
            onShow: function () {
                $('.btn-save-cancel-agn-address').text("Manage");
...
function saveAgencyDetails() {
    ...
    $.post(sAjaxAgencies, {
        ...
        if (rs.data.result == "1") {
            ...
            $("#frm-agn-branch input").each(function () {
                $(this).attr("oldVal", $(this).val());
                $(this).attr("value", $(this).val());
            });

            $('#service_dropdown').attr("oldVal", $('#service_dropdown').val())
            $('#inp-branch_status').attr("oldVal", $('#inp-branch_status').val())

            icmsMessage({
                type: "msgSuccess",
                body: "You have successfully updated the agency branch",
                onShow: function () {
                    $('.btn-edit-cancel-agn-details').text("Manage");
...
function loadContactAgencyList() {
    if (localStorage.getItem("reloadPage") == "1") {
        //do nothing
    } else {
        icmsMessage({
            type: "msgPreloader",
    ...
    $.post(sAjaxAgencies, {
        ...
        if (rs.data.result == "1") {
            ...
            $.each(rs.data.list, function (key, val) {
                ...
                if (val.agency_contact_is_primary != '1') {
                    list += '';
                } else {
                    $('.div-is_primary').hide();
                    list += '<span style="padding-left:10px;"> <i class="fas fa-star txt-orange"></i> </span>';
                }
                ...
                list += '                <a class="dropdown-item mng-validate_id" cont-prime="' + val.agency_contact_is_primary + '" addressid="' + val.address.address_list_id + '" email="' + val.agency_contact_email + '" tel="' + val.agency_contact_telephone_number + '" mob="' + val.agency_contact_mobile_number + '" lname="' + val.agency_contact_lastname + '" mname="' + val.agency_contact_middle_name + '" fname="' + val.agency_contact_firstname + '" dataid="' + val.agency_contact_id + '">Manage contact info</a>';
...
//        $('#mdl-manage-cont-address').modal('show');
    ...
    $('.btn-edit-cancel-agn-details').click(function () {
        var caption = $('.btn-edit-cancel-agn-details').text();
        if (caption == "Manage") {
            iniDetails['agency_details'] = getFormValues('frm-agn-branch');
            $('.btn-edit-cancel-agn-details').text("Cancel");
            ...
            var current = getFormValues('frm-agn-branch');
            if (current != iniDetails['agency_details']) {
                icmsMessage({
                    type: "msgConfirmation",
                    title: "Your changes will be lost when you canceled editing, do you want to continue",
                    body: "Click Ok button if you wish to continue.",
                    LblBtnConfirm: "Ok",
                    onConfirm: function () {
                        ...
                        resetFormJQueryValidation('frm-agn-branch');

                        $('#service_dropdown').val($('#service_dropdown').attr("oldVal"))
                        $('#inp-branch_status').val($('#inp-branch_status').attr("oldVal")).change();
    ...
    $('.btn-save-cancel-agn-address').click(function () {
        var caption = $('.btn-save-cancel-agn-address').text();
        if (caption == "Manage") {
            $('.btn-save-cancel-agn-address').text("Cancel");
            enabledAddress();
            iniDetails['address'] = getFormValues('frm-agency-address');
        } else {
            var current = getFormValues('frm-agency-address');
            if (current != iniDetails['address']) {
                icmsMessage({
                    type: "msgConfirmation",
                    title: "Your changes will be lost when you canceled editing, do you want to continue",
                    body: "Click Ok button if you wish to continue.",
                    ...
                    onConfirm: function () {
                        ...
                        $("#frm-agency-address select").each(function () {
                            var oldVal = $(this).attr("dataid");
                            $(this).val(oldVal).change();
                        });

                        $("#frm-agency-address textarea").each(function () {
                            var oldVal = $(this).attr("dataid");
                            $(this).val(oldVal);
                        });

                        resetFormJQueryValidation('frm-agency-address');
