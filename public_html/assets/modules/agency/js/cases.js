...
//        l += '            <div class="col-lg-8 col-md-8 col-sm-8  bk-trig-cmodal" attr-id="' + val.case_id + '">';
        // with out status 
        l += '            <div class="col-lg-10 col-md-9 col-sm-9 col-xs-9" attr-id="' + val.case_id + '">';
        ...
        l += '                        <span class="tags ' + getPriorityTag(val.case_priority_level_id) + '"></span> <span class="case-id">' + val.case_number + '</span>';
        l += '                        <br>';
        ...
        l += '                <div class="btn-group ellipse-action " data-id = "' + val.case_id + '">';
        l += '                    <a class="a-ellipse a-ellipse-i action_btn a-ellipse-' + val.case_id + ' "> <i class="fa fa-ellipsis-v" aria-hidden="true"></i> </a>';
        l += '                    <div class="action-menu action-' + val.case_id + '" data-cur-stat="1" style="display: none;">';
        l += '                        <a class="dropdown-item disabled action-title" href="#">Select Action</a>';
        l += '                        <a class="dropdown-item lvl-ch " href="' + sAjaxUrl + '/update_case/' + val.case_id + '">Manage Report</a>';
        l += '                        <a class="dropdown-item lvl-ce a-view_services" href="' + sAjaxUrl + '/view_victim_services/' + val.case_id + '/' + val.victim_id + '">Service Monitoring</a>';
        l += '                    </div>';
        l += '                </div>';
        l += '            </div>';
        l += '        </div>';
        ...
        if (rs.data.flag == 1) {
            ...
            switch (parseInt(x.tab)) {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    $('#list-tagged_user').html(buildCasesList(rs.data.content.listing));
                    $('.filter_load_placeholder').hide();
            ...
            var sFooter = '<a href="' + sAjaxUrl + '/add_case"> <button type="button" class="btn" style="background-color: #e88f15; color: #fff;"> ADD CASE </button> </a>';
            var sMessage = 'SORRY, WE COULDN\'T FIND ANY CASE RELATED TO <span class = "font-italic" >"<span>' + keyword + '</span>"</span>.';
