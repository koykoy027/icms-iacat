function buildCaseFilingActivitiesTemplate(res) {
    ...
    $.each(res, function(key, val) {
        ...
        l += '        <span class="tags t_medium"></span> <span class="case-id">' + val.case_number + '</span><br>';
        l += '        <span>Created by : </span><span class="icms-btn-secondary">' + val.filed_by_agency + '</span> <br>';
        l += '        <span>Date created : </span><span class="icms-btn-secondary">   ' + val.case_date_added + '</span>';
...
function getTopActiveAgency() {
    ...
    $.post(sAjaxDashboard, {
        ...
        $.each(rs.data.top_active_agency, function(key, val) {
            ...
            l += '         <span class="icms-btn-secondary p-r-10 "> <i class="fa fa-trophy p-r-10 txt-orange" aria-hidden="true"></i>' + val.case_count + ' Case</span>';
            l += '     </div>';
            l += ' </li>';
        });
...
function getUserActivityLogs(page) {
    ...
    $.post(sAjaxDashboard, {
        type: "getActivityLogs_branchUser",
        ...
        if (rs.data.result.count >= 1) {
            ...
            if (rs.data.result.count <= (page * limit_count) && rs.data.result.count >= limit_start) {
                ...
                $('#act-logs-content').append("<center>End of user logs</center>");
            }
        } else {
            if ($('#act-logs-content').attr('datapageend') == "0") {
                $('#list-all_logs').append("<li><hr><center>No user logs found<li></center>");
            } else {
                $('#list-all_logs').append("><li><hr><center>End of user logs<li></center>");
            }
            $('#act-logs-content').attr('datapageend', 1);
        }
    }, 'json');
